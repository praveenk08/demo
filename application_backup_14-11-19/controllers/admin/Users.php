...
class Users extends MY_Controller {
	...
	public function saveServiceCategory(){
		...
		$response['status']=TRUE;
 		$name_error='';
  		$this->form_validation->set_rules('name', 'Service Category Name', 'trim|required');
	...
	{
		$where=array('sc.status'=>1,'sc.is_deleted'=>0);
		$data['service_categories']=$this->Common_model->getServicesCategories($where);
	...
	public function addUpdateService(){
 	...
 	public function saveService(){
		...
		if ($this->form_validation->run() == FALSE || $response['status']==FALSE) {
			...
			$slug=makeSlug($save_data['name'].'-'.$service_id);
