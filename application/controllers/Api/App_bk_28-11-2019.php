<?php
	...
	class App extends REST_Controller{
		...
		function providerServiceCategory_POST(){
			...
			$response['status']=TRUE;
			$where=array('status'=>1,'is_deleted'=>0);
			$response['response']=$this->App_model->getSingleTableData('service_category',$where,array('key'=>'id','value'=>'DESC'));
...
// Resend OTP
	...
	function weatherForecast_POST(){
 		if(isset($_POST) && $this->input->post('vendor_id')>0){
				$response['response']=(object)array();
				$response['status']=TRUE;
				$vendor_id=$this->input->post('vendor_id');
				$where=array('user_id'=>$vendor_id);
		...
		public function getVendorOrderDetails_POST(){
			if(isset($_POST) && $this->input->post('vendor_id')>0 && !empty($this->input->post('order_id'))){
				...
				$vendor_id=$this->input->post('vendor_id');
				$where=array('od.vendor_id'=>$this->input->post('vendor_id'),'od.order_id'=>$order_id);
