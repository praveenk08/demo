...
class Deliveryboy extends DeliveryboyController {
	...
	public function updateProfile(){
		...
		$this->form_validation->set_rules('delivery_boy_first_name', 'First Name', 'trim|required');
 		$this->form_validation->set_rules('delivery_boy_phone', 'Phone Number', 'required');
	...
	public function assignedOrders(){
		...
		$totalRec = count($this->Deliveryboy_model->assignedOrders($param));
		$config=ajaxPaginationConfigration(array('base_url'=>base_url().'website/Deliveryboy/assignedOrdersAjax','totalRec'=>$totalRec,'per_page'=>getSettings()['total_record_per_page']));
	...
	public function assignedOrdersAjax(){
		$param['where']=array('dboa.delivery_boy_id'=>$this->session->userdata('delivery_boy_data')['id']);
		$param['where_in']=array(3,4);
		...
		foreach($orders as $order){
 		...
 		$this->load->view('website/delivery-boy/delivery-boy-assigned-orders-ajax',$data,false);
	}
	...
	public function changeOrderStatus(){
		...
		if($id>0){
			...
			$this->db->update('orders',array('order_status'=>$status));
 			$response=array('status'=>1,'message'=>"Order Status Updated Successfully!");
			echo json_encode($response);
		}
	}
 }
