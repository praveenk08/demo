...
class Team extends MY_Controller {
	...
	function uploadTeamImage()
	{
		...
		if(!file_exists('./attachments/teams/'.$this->session->userdata('admin_data')['id'])){
			if(mkdir('./attachments/teams/'.$this->session->userdata('admin_data')['id'], 777));
		}
		$filepath = './attachments/teams/'.$this->session->userdata('admin_date')['id'].'/'.$filename;
		$blob= $image_parts[1];
	...
	public function addUpdateTeam(){
		...
		if($id>0){
			$select=array('id'=>$id);
			$data['team']=$this->Page_model->teamData($select);
