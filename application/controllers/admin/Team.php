...
function uploadTeamImage()
{
		...
		if($id>0){
			...
			$teamdata=getSingleTableData('our_team',$select);
			 if(count($teamdata)>0){
				...
				$team_name_data=$this->Page_model->getOurTeamData(array('ot.id'=>$id));
				if(count($team_name_data)>0){
					foreach($team_name_data as $team_name_row){
						$data['names'][$team_name_row['abbr']]=$team_name_row;
					}	
				}
			}
			else
	...
	public function viewTeam(){
 	...
 	public function saveTeam(){
		...
		$this->form_validation->set_rules('nameen', 'Name', 'trim|required');
      ...
      if($this->form_validation->run() == TRUE && $response['status']==TRUE){
		...
		*/
		...
		if ($this->form_validation->run() == FALSE || $response['status']==FALSE) {
			...
			$team_id=$this->Common_model->AddUpdateData('our_team',$save_data);


			$languages=$this->input->post('languages');
			if(count($languages)){
				$this->Common_model->deleteRecord(array('team_id'=>$team_id),'our_team_translator');
				foreach($languages as $language){
					$save_data=array(
					'id'=>0,
					'team_id'=>$team_id,
					'name'=>$this->input->post('name'.$language),
					'designation'=>$this->input->post('designation'.$language),
					'description'=>$this->input->post('description'.$language),
					'abbr'=>$language,
 					);
					$id=$this->Common_model->AddUpdateData('our_team_translator',$save_data);
					$slug=makeSlug($save_data['name'].'-'.$id);
					$update_data=array('id'=>$id,'slug'=>$slug);
					$this->Common_model->AddUpdateData('our_team_translator',$update_data);
					}
				}
		...
		if($id>0){
			...
			$servicedata=getSingleTableData('our_services',$select);
			 if(count($servicedata)>0){
				...
				$service_name_data=$this->Page_model->getOurServicesData(array('os.id'=>$id));
 ...
 */
		...
		if ($this->form_validation->run() == FALSE || $response['status']==FALSE) {
			...
			$service_id=$this->Common_model->AddUpdateData('our_services',$save_data);
			
			$languages=$this->input->post('languages');
			if(count($languages)){
				$this->Common_model->deleteRecord(array('service_id'=>$service_id),'our_services_translator');
				foreach($languages as $language){
					$save_data=array(
					'id'=>0,
					...
					$id=$this->Common_model->AddUpdateData('our_services_translator',$save_data);
					$slug=makeSlug($save_data['name'].'-'.$id);
					$update_data=array('id'=>$id,'slug'=>$slug);
					$this->Common_model->AddUpdateData('our_services_translator',$update_data);
					}
				}
			if($save_data['id']>0){
				$id=$save_data['id'];
	...
	if($id>0){
		...
		$work_processdata=getSingleTableData('work_process',$select);
			 if(count($work_processdata)>0){
				...
				$work_process_data=$this->Page_model->getWorkProcessData(array('wp.id'=>$id));
				if(count($work_process_data)>0){
					foreach($work_process_data as $work_process_row){
						$data['names'][$work_process_row['abbr']]=$work_process_row;
					}	
				}
			}
			else
...
public function viewWorkProcess(){
 ...
 public function saveWorkProcess(){
	...
	if ($this->form_validation->run() == FALSE || $response['status']==FALSE) {
		...
		$languages=$this->input->post('languages');
		if(count($languages)){
			$this->Common_model->deleteRecord(array('process_id'=>$id),'work_process_translator');
			foreach($languages as $language){
				$save_data=array(
				'id'=>0,
				'process_id'=>$id,
				...
				$this->Common_model->AddUpdateData('work_process_translator',$save_data);
    			}
			}
	...
	if($id>0){
		...
		$facilitydata=getSingleTableData('facilities',$select);
			 if(count($facilitydata)>0){
				...
				$facility_name_data=$this->Page_model->getFacilityNameData(array('f.id'=>$id));
				if(count($facility_name_data)>0){
					foreach($facility_name_data as $facility_name_row){
						$data['names'][$facility_name_row['abbr']]=$facility_name_row;
					}	
				}
			}
			else
 ...
 public function saveFacility(){
	...
	*/
	...
	//	}
		...
		$languages=$this->input->post('languages');
			if(count($languages)){
				$this->Common_model->deleteRecord(array('facility_id'=>$id),'facilities_translator');
				foreach($languages as $language){
					$save_data=array(
						'id'=>0,
						'facility_id'=>$id,
					...
					$this->Common_model->AddUpdateData('facilities_translator',$save_data);
 					}
				}
		if($save_data['id']>0){
			$id=$save_data['id'];
