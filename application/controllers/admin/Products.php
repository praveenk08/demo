...
class Products extends MY_Controller {
	...
	public function index()
	{
		...
		$data['units']=getUnitList($where);
 		$where=array('u.status'=>1,'u.is_deleted'=>0,'u.role_id'=>2);
	...
	public function addUpdateProduct(){
		deleteTempFiles('./attachments/products/'.$this->session->userdata('admin_data')['id'].'/*');
 		$data=array();
			...
			$data['names']=array();
		if($id>0){
			$select=array('vp.id'=>$id,'ct.abbr'=>'en');
			...
			$names=$this->Products_model->productNamedata(array('vp.id'=>$id));
			if(count($names)>0){
