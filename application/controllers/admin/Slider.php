...
class Slider extends MY_Controller {
	...
	public function sliders()
	{
		...
		{
			$slider = $this->Category_model->manageSlidersExport($_POST,false);
        
        $filename = 'slider_list'.date('d-m-Y m-i-s').'.csv'; 
	...
	function uploadSliderImage(){
		$this->isAjaxRequest();
		$response=uploadCroperImage('sliders',$this->session->userdata('admin_data')['id'],$this->input->post('image'));
	...
	public function viewSlider(){
  	...
  	public function saveSlider(){
		...
		if(empty($old_image) && empty($slider_image)){
	...
	{
		$managefaq = $this->Category_model->manageFaqsExport($_POST,false);
        //echo"<pre>"; print_r($managefaq);die;
        $filename = 'Faqs_list'.date('d-m-Y m-i-s').'.csv'; 
        header("Content-Description: File Transfer"); 
        header("Content-Disposition: attachment; filename=$filename"); 
        header("Content-Type: application/csv; ");
	...
	public function addUpdateFaq(){
		...
		if($id>0){
			...
			$faqdata=getSingleTableData('faq',$select);
			 if(count($faqdata)>0){
				$data['faq']=$faqdata[0];
				 $faq_data=$this->Category_model->getFaqNameData(array('f.id'=>$id));
				 
				if(count($faq_data)>0){
					foreach($faq_data as $faq_data_row){
 						$data['names'][$faq_data_row['abbr']]=$faq_data_row;
					}	
				}
			}
			else
	...
	public function saveFaq(){
		...
		$response['status']=true;
		 ...
		 if(count($languages)){
			 foreach($languages as $language){
				$this->form_validation->set_rules('question'.$language['abbr'], 'Question ('.ucfirst($language['name']).')', 'trim|required');
				$this->form_validation->set_rules('answer'.$language['abbr'], 'Answer ('.ucfirst($language['name']).')', 'trim|required');
			}
		 }
		...
		if ($this->form_validation->run() == FALSE || $response['status']==FALSE) {
			...
			$save_data=array(
 			...
 			$slug=makeSlug($this->input->post('questionen').'-'.$faq_id);
			$update_data=array('id'=>$faq_id,'slug'=>$slug);
			$this->Common_model->AddUpdateData('faq',$update_data);

			$languages=$this->input->post('languages');
			if(count($languages)){
				$this->Common_model->deleteRecord(array('faq_id'=>$faq_id),'faq_translator');
				foreach($languages as $language){
					$save_data=array(
					'id'=>0,
					'faq_id'=>$faq_id,
					'question'=>$this->input->post('question'.$language),
					'answer'=>$this->input->post('answer'.$language),
 					'abbr'=>$language,
 					);
					$this->Common_model->AddUpdateData('faq_translator',$save_data);
					}
				}
