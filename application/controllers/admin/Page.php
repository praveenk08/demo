...
class Page extends MY_Controller {
		...
		if($id>0){
			...
			$pagedata=getSingleTableData('cms_pages',$select);
			 if(count($pagedata)>0){
				...
				$page_name_data=$this->Page_model->getPageNameData(array('p.id'=>$id));
				if(count($page_name_data)>0){
					foreach($page_name_data as $page_name_row){
						$data['names'][$page_name_row['abbr']]=$page_name_row;
					}	
				}
 			}
 	...
 	public function savePage(){
		...
		if(count($languages)){
			foreach($languages as $language){
				$this->form_validation->set_rules('name'.$language['abbr'], 'Page Name ('.ucfirst($language['name']).')', 'trim|required');
				$this->form_validation->set_rules('meta_title'.$language['abbr'], 'Meta Title ('.ucfirst($language['name']).')', 'trim|required');
				$this->form_validation->set_rules('meta_keywords'.$language['abbr'], 'Meta Keywords ('.ucfirst($language['name']).')', 'trim|required');
				$this->form_validation->set_rules('banner_title'.$language['abbr'], 'Banner Title ('.ucfirst($language['name']).')', 'trim|required');
				$this->form_validation->set_rules('description'.$language['abbr'], 'Page Description ('.ucfirst($language['name']).')', 'trim|required');
			}
		}
		...
		if ($this->form_validation->run() == FALSE || $response['status']==FALSE) {
			...
			if(count($languages)){
				$this->Common_model->deleteRecord(array('page_id'=>$page_id),'page_translator');
				foreach($languages as $language){
					$save_data=array(
						'id'=>0,
						'page_id'=>$page_id,
						...
						'abbr'=>$language,
						'meta_title'=>$this->input->post('meta_title'.$language),
						'meta_keywords'=>$this->input->post('meta_keywords'.$language),
						'meta_description'=>$this->input->post('meta_description'.$language),
						'description'=>$this->input->post('description'.$language),
						'banner_title'=>$this->input->post('banner_title'.$language),
 					);
			...
			$slug=makeSlug($this->input->post('nameen').'-'.$page_id);
	...
	public function updateWebsiteSettings(){
		...
		$website_logo_error='';
 		...
 		$product_banner_error='';
 		$arabic_logo_error='';
 		$service_banner_error='';
		...
		$old_product_banner=$this->input->post('old_product_banner');
		$old_arabic_logo=$this->input->post('old_arabic_logo');
		$old_service_banner=$this->input->post('old_service_banner');
	   ...
	   if($this->form_validation->run() == TRUE && $response['status']==TRUE){
	    ...
	    if($this->form_validation->run() == TRUE && $response['status']==TRUE){
		   ...
		   if(!empty($_FILES['arabic_logo']['name'])){
	 
				$allowed_types = 'png|jpg|jpeg|gif';
				$input_name = 'arabic_logo';
				$file_response=uploadImage('pages',$allowed_types,$input_name,$old_arabic_logo);
				 if($file_response['status']){
					$arabic_logo=$file_response['name'];
				}else{
					$response['status']=false;
					$arabic_logo_error=$file_response['message'];
				}

		}else{
			$arabic_logo=$old_arabic_logo;
		}
	   } 
	    ...
	    if($this->form_validation->run() == TRUE && $response['status']==TRUE){
		   ...
		   if(!empty($_FILES['product_banner']['name'])){
				...
				$allowed_types = 'png|jpg|jpeg|gif';
				$input_name = 'product_banner';
				$file_response=uploadImage('pages',$allowed_types,$input_name,$old_product_banner);
				 if($file_response['status']){
					$product_banner=$file_response['name'];
				}else{
					$response['status']=false;
					$product_banner_error=$file_response['message'];
				}

		}else{
			$product_banner=$old_product_banner;
		}
	   } 
	    ...
	    if($this->form_validation->run() == TRUE && $response['status']==TRUE){
		   ...
		   if(!empty($_FILES['service_banner']['name'])){
				...
				$allowed_types = 'png|jpg|jpeg|gif';
				$input_name = 'service_banner';
				$file_response=uploadImage('pages',$allowed_types,$input_name,$old_service_banner);
				 if($file_response['status']){
					$service_banner=$file_response['name'];
				}else{
					$response['status']=false;
					$service_banner_error=$file_response['message'];
				}

		}else{
			$service_banner=$old_service_banner;
		}
	   } 
		...
		if ($this->form_validation->run() == FALSE || $response['status']==FALSE) {
			...
			if(!empty($arabic_logo_error)){
				$response['arabic_logo']=$arabic_logo_error;
			}
			if(!empty($product_banner_error)){
				$response['product_banner']=$product_banner_error;
			}
			if(!empty($service_banner_error)){
				$response['serice_banner']=$service_banner_error;
			}
			$response['message']='There is error in submitting form!';
		}else{
			$save_data=array(
			...
			if(!empty($arabic_logo)){
				$save_data['arabic_logo']=$arabic_logo;
			}
			if(!empty($product_banner)){
				$save_data['product_banner']=$product_banner;
			}
			if(!empty($service_banner)){
				$save_data['service_banner']=$service_banner;
			}
	...
	public function removeLoginRegistrationBanner(){
		...
		public function removeArabicLogo(){
		$this->isAjaxRequest();
		$id=1;
		$image=$this->input->post('old_arabic_logo');
		if(!empty($id) && !empty($image)){
			$update_data=array(
				'arabic_logo'=>'',
				'id'=>$id
			...
			unlinkImage('pages',$image);
			if($result){
				$response['message']="Arabic Logo deleted successfully!";
				$logdata=array(
					'activity_in'=>1,
					'category'=>16,
					'reference_id'=>$id,
	...
	// 	public function removeServiceBanner(){
	// 	$this->isAjaxRequest();
	// 	$id=1;
	// 	$image=$this->input->post('old_service_banner');
	// 	if(!empty($id) && !empty($image)){
	// 		$update_data=array(
	// 			'service_banner'=>'',
	// 			'id'=>$id
 ...
 // 			$result=$this->Common_model->AddUpdateData('website_settings',$update_data);
	// 		unlinkImage('pages',$image);
	// 		if($result){
	// 			$response['message']="Service Banner deleted successfully!";
	// 			$logdata=array(
	// 				'activity_in'=>1,
	// 				'category'=>16,
	// 				'reference_id'=>$id,
	...
	public function removeProductBanner(){

		$this->isAjaxRequest();
		$id=1;
		$image=$this->input->post('old_product_banner');
		...
		if(!empty($id) && !empty($image)){
			...
			unlinkImage('pages',$image);
			if($result){
				$response['message']="Product Banner deleted successfully!";
				$logdata=array(
					'activity_in'=>1,
					'category'=>16,
					'reference_id'=>$id,
   ...
   public function addUpdateCalculation(){
	   ...
	   if($id>0){
		   $select=array('id'=>$id);
		   $calculationData=getSingleTableData('calculations',$select,array('key'=>'id','value'=>'ASC'));
		   if(count($calculationData)){
			$data['calculation']=$calculationData[0];
			$calculation_name_data=$this->Page_model->getCalculationData(array('c.id'=>$id));
			 
			if(count($calculation_name_data)>0){
				foreach($calculation_name_data as $calculation_name_row){
					$data['names'][$calculation_name_row['abbr']]=$calculation_name_row;
				}	
			}
		   }else{
   ...
   public function saveCalculation(){
	  ...
	  // $this->form_validation->set_rules('title', 'Title', 'trim|required');
	   ...
	   if ($this->form_validation->run() == FALSE || $response['status']==FALSE) {
		   ...
		   $languages=$this->input->post('languages');
			if(count($languages)){
				$this->Common_model->deleteRecord(array('calculation_id'=>$calculation_id),'calculation_translator');
				foreach($languages as $language){
					$save_data=array(
					'id'=>0,
					...
					$id=$this->Common_model->AddUpdateData('calculation_translator',$save_data);
					$slug=makeSlug($save_data['name'].'-'.$id);
					$update_data=array('id'=>$id,'slug'=>$slug);
					$this->Common_model->AddUpdateData('calculation_translator',$update_data);
					}
				}
