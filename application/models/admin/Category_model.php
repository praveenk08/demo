<?php class Category_model extends CI_Model
{
	...
	public function categoryNameData($where){
		if(count($where)>0){
			$this->db->where($where);
			$this->db->select('ct.name,ct.abbr');
	...
	public function manageCategoriesAjax($post,$count=false){
		...
		$this->db->select('c.*,ct.name,ct2.name as parent_name');
		$this->db->from('category c');
	...
	public function manageUnitsAjax($post,$count=false){
		...
		$this->db->select("u.id,ut.name,u.status,DATE_FORMAT(u.added_date,'%d-%m-%Y %H:%i') as added_date");
		$this->db->from('units u');
	...
	public function manageFaqsAjax($post,$count=false){
		...
		$this->db->select('f.*,ft.question,ft.answer');
		$this->db->from('faq f');
		$this->db->join('faq_translator ft','f.id=ft.faq_id','INNER');
 		if(count($where)>0){
			$this->db->where($where);
		}
		if(count($like)>0){
 ...
 public function manageFaqsExport($post,$count=false){
		...
		$this->db->select('ft.question ,ft.answer , f.status ,f.added_date');
...
public function getUnitNameData($where){
	if(count($where)>0){
		$this->db->where($where);
		$this->db->select("ut.name,ut.abbr");
		$this->db->from('units u');
...
public function getFaqNameData($where){
	if(count($where)>0){
 		$this->db->where($where);
		$this->db->select("ft.question,ft.answer,ft.abbr");
		$this->db->from('faq f');
		$this->db->join('faq_translator ft','f.id=ft.faq_id','INNER');
		$result=$this->db->get();
		$response=$result->result_array();
